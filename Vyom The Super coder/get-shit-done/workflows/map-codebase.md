...
```bash
INIT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-codebase-mapper 2>/dev/null)
```
...
<step name="sequential_mapping" condition="Task tool is NOT available (e.g. Vyom, Gemini CLI, Codex)">
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
