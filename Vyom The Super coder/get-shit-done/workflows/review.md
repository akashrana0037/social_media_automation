...
if [ "$Vyom_AGENT" = "1" ]; then
  # Vyom is a separate client — all CLIs are external, skip none
  SELF_CLI="none"
...
Commit:
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
