...
```bash
INIT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-doc-writer 2>/dev/null)
```
...
<step name="sequential_generation" condition="Task tool is NOT available (e.g. Vyom, Gemini CLI, Codex, Copilot)">
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "docs: generate project documentation" \
